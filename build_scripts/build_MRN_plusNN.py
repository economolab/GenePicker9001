...
scRNAseq_data = data_nav.fetch_scRNAseq(clusters, scRNAseq_meta, form='raw', neur_frac=1, nn_frac=0.07)
...
np.save(os.path.join(params.local_data_dir, "MRN-scRNAseq-NN-raw"), scRNAseq_raw)

# save scRNAseq metadata
scRNAseq_meta.to_csv(os.path.join(params.local_data_dir, "MRN-scRNAseq-NN-meta.csv"),
                                  index=False)

# save MERFISH frequencies
# MERFISH_freqs = cell_funcs.calc_frac_per_type(MERFISH_meta)
with open(os.path.join(params.local_data_dir, "MRN-MERFISH-NN-freqs.pkl"), 'wb') as handle:
...
np.save(os.path.join(params.local_data_dir, "MRN-scRNAseq-NN-norm"), exp_norm)
