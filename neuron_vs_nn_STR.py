...
meta = pd.read_csv(os.path.join(params.local_data_dir, "STR-scRNAseq-NN-meta.csv"), low_memory=False)
exp = np.load(os.path.join(params.local_data_dir, "STR-scRNAseq-NN-raw.npy"))
freqs = pd.read_pickle(os.path.join(params.local_data_dir, "STR-MERFISH-NN-freqs.pkl"))
...
# non_neuronal = ['334 Microglia NN']
# non_neuronal = ['318 Astro-NT NN', '319 Astro-TE NN', '320 Astro-OLF NN']
...
ABC_plot.plot_gene("Nap1l5", meta_boot, exp_boot, level='cluster')
...
for j in tqdm(range(exp_boot.shape[1])):
    ...
    # average expression of gene in neurons
    neuron_exp.append(np.mean(exp_boot[neuron_index,j]))
    
    # average expression of gene in non-neurons
...
ABC_plot.plot_gene("Celf2", meta_boot, exp_boot, level='subclass')
